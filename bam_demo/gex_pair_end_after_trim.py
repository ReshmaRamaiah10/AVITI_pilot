...
files = {
    "Ch10": {
        "Illumina": {
            "R2": "ch10_illumina_qc.corrected.txt",
        },
        "AVITI": {
            "R2": "ch10_aviti_qc.corrected.txt",
        }
        },
    "Ch7": {
        "Illumina": {
            "R2": "ch7_illumina_qc.corrected.txt",
        },
        "AVITI": {
            "R2": "ch7_aviti_qc.corrected.txt",
        }
}}
...
for sample in files.keys():
    fig, axes = plt.subplots(2, 2, figsize=(13,5), gridspec_kw={'height_ratios': [95, 5], 'hspace': 0.02}, sharey=True, sharex=False)
    ...
    plot_bar("Illumina, R2", files[sample]["Illumina"]["R2"], axes[0][0], r2_regions, axes[1][0])
    plot_bar("AVITI, R2", files[sample]["AVITI"]["R2"], axes[0][1], r2_regions, axes[1][1])


    # Legend for quality bins
    axes[0][-1].legend(loc="right", bbox_to_anchor=(1.25, 0.8), title="Quality bin")
    ...
    axes[1][-1].legend(handles=region_legend_elements, loc="right", bbox_to_anchor=(1.25, 10), title="Region")

    plt.minorticks_off()
    axes[0][0].set_ylabel("Percent reads")
    fig.suptitle(f"{sample} GEX libraries (5')")
    fig.supxlabel("Position in read (bases)")
    fig.savefig(f"figure/{sample}.correctedR2.png", dpi=500)
    plt.close()
